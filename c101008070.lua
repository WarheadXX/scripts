...
function s.filter(c,tp,e)
	return c:IsSetCard(0x104f) and not c:IsCode(assualtNameCode) and c:IsCanBeSpecialSummoned(e,0,tp,true,false,POS_FACEUP_DEFENSE)
end
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then
		...
		e:SetLabel(0)
		return ft>-1 and Duel.CheckReleaseGroup(tp,s.cfilter,1,nil,ft)
	end
	-- local rg=Duel.SelectReleaseGroupCost(tp,s.cfilter,1,1,false,nil,nil,e,tp,ft)
	local rg=Duel.SelectReleaseGroup(tp,s.cfilter,1,1,nil,ft)
	Duel.SetTargetParam(rg:GetFirst():GetCode())
	assualtNameCode = rg:GetFirst():GetCode()
