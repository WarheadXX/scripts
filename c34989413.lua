...
function c34989413.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if e:GetLabel()==0 and Duel.IsExistingMatchingCard(c34989413.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil,lg,true) then
		...
		if lg:GetCount()>1 then
			for tc in aux.Next(lg) do
				if lg:IsExists(function(c,tc) return c:GetRace() and tc:GetRace()>0 end,lg:GetCount()-1,tc,tc) then
					if tc:GetRace() and share==0 then share=share+(share and not tc:GetRace()) end
				end
			end
		end
		val=Duel.AnnounceRace(tp,1,0xffffff)
	elseif e:GetLabel()~=0 and Duel.IsExistingMatchingCard(c34989413.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil,lg,false) then
		...
		if lg:GetCount()>1 then
			for tc in aux.Next(lg) do
				if lg:IsExists(function(c,tc) return c:GetAttribute() and tc:GetAttribute()>0 end,lg:GetCount()-1,tc,tc) then
					if tc:GetAttribute() and share==0 then share=share+(share and not tc:GetAttribute()) end
				end
			end
		end
		val=Duel.AnnounceAttribute(tp,1, 0xffffff)
	else return
	end
	for tc in aux.Next(lg) do
		local e1=Effect.CreateEffect(c)
