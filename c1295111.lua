...
-- function s.linkop(e,tp,eg,ep,ev,re,r,rp,c)
	...
	-- local rc=Duel.SelectMatchingCard(tp,s.rmfilter,tp,LOCATION_MZONE,0,1,1,nil,tp):GetFirst()
	-- if rc and Duel.SendtoGrave(rc,REASON_MATERIAL+REASON_LINK)>0 then
...
-- function s.lmfilter(c,lc,tp)
    -- return c:IsFaceup() and c:IsType(TYPE_LINK)
        -- and c:IsCode(lc:GetCode()) and c:IsCanBeLinkMaterial(lc,tp)
...
-- function s.linkcon(e,c)
    ...
    -- return ((mustg:GetCount()==1 and s.lmfilter(mustg:GetFirst(),c,tp)) or (mustg:GetCount()==0 and g:GetCount()>0))
...
-- function s.linktg(e,tp,eg,ep,ev,re,r,rp,chk,c)
    ...
    -- if mustg:GetCount()>0 then
        ...
        -- return true
    -- end
    -- local tc=g:Select(1,1,mustg):GetFirst()
    -- if tc then
