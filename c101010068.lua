...
function s.atkcost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if tgct==mg:GetCount() then ct=mg:GetCount()-1 end
	if ct==0 then ct=1 end
	--local g=aux.SelectUnselectGroup(mg,e,tp,1,ct,s.rescon,1,tp,HINTMSG_REMOVE,nil,nil)
	local g=Duel.SelectMatchingCard(tp,s.cfilter,tp,LOCATION_MZONE,0,1,ct,tp)
	if Duel.Remove(g,POS_FACEUP,REASON_COST+REASON_TEMPORARY)==g:GetCount() then
		g:KeepAlive()
		e:SetLabelObject(g)
