...
function c2347477.initial_effect(c)
	...
	e3:SetDescription(aux.Stringid(2347477,1))
	e3:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_F)
	e3:SetProperty(EFFECT_FLAG_DELAY+EFFECT_FLAG_CANNOT_DISABLE)
	e3:SetCode(EVENT_BE_MATERIAL)
	e3:SetCountLimit(1,2347478)
	e3:SetCondition(c2347477.matcon)
	e3:SetOperation(c2347477.matop)
	c:RegisterEffect(e3)
end
function c2347477.matcon(e,tp,sg,mg,lc,og,chk)
	return bit.band(e:GetHandler():GetPreviousLocation(),LOCATION_HAND)~=0 and microCoder == 2347477
end
function c2347477.matop(e,tp,eg,ep,ev,re,r,rp)
	    local ge1=Effect.CreateEffect(e:GetHandler())
		ge1:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
		ge1:SetCode(EVENT_PHASE+PHASE_END)
		ge1:SetCountLimit(1)
		ge1:SetCondition(Auxiliary.ResetCodeMaterial)
...
-- function c2347477.extracon(c,e,tp,sg,mg,lc,og,chk)
	...
	-- (sg:Filter(c2347477.flagcheck,nil):GetCount())<2
-- end
-- function c2347477.flagcheck(c)
	-- return c:GetFlagEffect(2347477)>0
-- end
function c2347477.extraval()
return true
end
-- function c2347477.extraval(chk,summon_type,e,...)
	...
	-- else
		-- local sg,sc,tp=...
		-- if summon_type == SUMMON_TYPE_LINK and sg:GetCount()>0 then
