...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.IsExistingMatchingCard(aux.FilterFaceupFunction(Card.IsNonEffectMonster),tp,LOCATION_MZONE,0,1,nil) and g:GetCount()>0 and Duel.SelectYesNo(tp,aux.Stringid(id,1)) then
	--if  g:GetCount()>0 and Duel.SelectYesNo(tp,aux.Stringid(id,1)) and Duel.IsExistingMatchingCard(noneffectfilter,tp,LOCATION_MZONE,0,1,nil) then
		Duel.BreakEffect()
