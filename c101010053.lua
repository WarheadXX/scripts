...
function s.matchk(e,c)
	if c:GetMaterial():IsExists(Card.IsCode,1,nil,101010040) then
...
function s.eqfil(c)
    return Duel.GetMatchingGroup(aux.NecroValleyFilter(s.eqfilter),tp,LOCATION_GRAVE,0,nil)
end
function s.eqop(e,tp,eg,ep,ev,re,r,rp)
	...
	--local g=Duel.SelectMatchingCard(tp, s.eqfil,tp,LOCATION_GRAVE,0,nil),e,tp,1,3,aux.dncheck,1,tp,HINTMSG_EQUIP)
	local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.eqfilter),tp,LOCATION_GRAVE,0,1,3,tp)
