...
function c99011763.matcheck(e,c)
	...
	if g:GetCount()==2 and g:GetClassCount(Card.GetCode)==g:GetCount() and not g:IsExists(aux.NOT(Card.IsAttribute),1,nil,ATTRIBUTE_DARK) then
		local e1=Effect.CreateEffect(c)
...
function c99011763.rfilter(c)
	return c:IsType(TYPE_MONSTER)
end
function c99011763.addcost(e,tp,eg,ep,ev,re,r,rp,chk)
    local lg=e:GetHandler():GetLinkedGroup()
	if chk==0 then return Duel.CheckReleaseGroup(tp,c99011763.rfilter,1,nil) end
	local g=Duel.SelectReleaseGroup(tp,c99011763.rfilter,1,1,nil)
