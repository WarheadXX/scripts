...
function s.check(g)
	return  function(sg,tp)
				...
				return aux.ChkfMMZ(sg:GetCount())(sg,nil,tp) 
					and g:Filter(aux.NOT(Card.IsCode),nil,table.unpack(codes)):GetClassCount(Card.GetCode)>=sg:GetCount()
			end
end
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,rg:GetCount(),tp,LOCATION_HAND)
end
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	local ct=math.min(rg:GetCount(),Duel.GetLocationCount(tp,LOCATION_MZONE))
	if g:GetCount()>0 and ct>0 then
		...
		local sg=Duel.SelectMatchingCard(tp,s.filter,tp,LOCATION_HAND,0,ct,ct,nil)
