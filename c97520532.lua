...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc then
		...
		if fg:GetCount()>0 and fg:FilterCount(s.spcfilter,1,nil)==fg:GetCount() 
			and hg:GetCount()>0 and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.SelectYesNo(tp,aux.Stringid(id,2)) then
			Duel.BreakEffect()
