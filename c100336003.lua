...
function s.cfilter(c,tp)
	return c:IsAttribute(ATTRIBUTE_DARK) --c:IsPreviousControler(tp)
		and aux.FilterEqualFunction(Card.GetSummonLocation,LOCATION_EXTRA) and c:IsReason(REASON_BATTLE+REASON_EFFECT)
end
...
function s.spfilter(c,oc,e,tp)
	return not c:IsCode(c:GetCode()) and c:IsAttribute(ATTRIBUTE_DARK) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
...
function s.imfilter(c)
	return c:IsFaceup() and c:IsAttribute(ATTRIBUTE_DARK) and aux.FilterEqualFunction(c.GetSummonLocation,LOCATION_EXTRA)
end
function s.imcon(e)
