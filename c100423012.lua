...
function s.synop(e,tp,eg,ep,ev,re,r,rp)
    ...
    if g:FilterCount(s.relfilter,nil,e,tp)<g:GetCount() then return end
...
function s.xyzfilter(c,mg)
    return c:IsSetCard(0x231) and c:IsType(TYPE_XYZ) and (not mg or c:IsXyzSummonable(mg,mg:GetCount(),mg:GetCount()))
end
function s.xrescon(exg)
    return function(sg,e,tp,mg)
        return aux.dncheck(sg,e,tp,mg) and exg:IsExists(Card.IsXyzSummonable,1,nil,sg,sg:GetCount(),sg:GetCount())
    end
end
...
function s.xyzop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<g:GetCount() or g:GetCount()==0 or (Duel.IsPlayerAffectedByEffect(tp,CARD_BLUEEYES_SPIRIT) and g:GetCount()>1) then return end
	for tc in aux.Next(g) do
		Duel.SpecialSummonStep(tc,0,tp,tp,false,false,POS_FACEUP)
		local e1=Effect.CreateEffect(e:GetHandler())
		e1:SetType(EFFECT_TYPE_SINGLE)
	...
	if Duel.GetLocationCountFromEx(tp,tp,g)<=0 then return end
	Duel.BreakEffect()
	local xyzg=Duel.GetMatchingGroup(s.xyzfilter,tp,LOCATION_EXTRA,0,nil,g,g:GetCount())
