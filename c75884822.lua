...
function c75884822.repfilter(c,tp)
	return c:IsFaceup() and c:IsControler(tp) and c:IsLocation(LOCATION_ONFIELD)
		and ((c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsRace(RACE_MACHINE) and c:IsType(TYPE_FUSION)) or (c:IsCode(59464593,980973,46384672,73879377,65192027,89189982) or c:IsSetCard(0x111)))
...
function c75884822.desfilter(c,e,tp)
	return (c:IsFaceup() and c:IsLocation(LOCATION_ONFIELD) and c:IsSetCard(0xf) and c:IsAbleToRemove() and not c:IsStatus(STATUS_DESTROY_CONFIRMED+STATUS_BATTLE_DESTROYED) and c:IsControler(tp)) 
	    or (c:IsControler(tp) and c:IsLocation(LOCATION_HAND+LOCATION_GRAVE) and c:IsSetCard(0xf)
