...
function s.spcfil1(c,tp)
	return c:IsSetCard(0x2016) and Duel.IsExistingMatchingCard(s.spcfil2,tp,LOCATION_DECK,0,1,c,c:GetLevel())
end
function s.spcfil2(c,lv)
	return c:IsSetCard(0x2016) and c:GetLevel()+lv==lvs
end
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetMatchingGroup(s.filter,tp,LOCATION_DECK,0,nil)
	-- local sg=aux.SelectUnselectGroup(g,e,tp,2,2,s.rescon(e:GetLabel()),1,tp,HINTMSG_SELECT)
	local sg=Duel.SelectMatchingCard(tp,s.spcfil1,tp,LOCATION_DECK,0,1,1,nil,tp)
	local tc1=sg:GetFirst()
	-- lv = tc1:GetLevel()
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local g2=Duel.SelectMatchingCard(tp,s.spcfil2,tp,LOCATION_DECK,0,1,1,tc1,tc1:GetLevel())
	sg:Merge(g2)
	lvs=nil
