...
function c52653092.initial_effect(c)
	...
	e1:SetCondition(c52653092.spcon)
	--e1:SetTarget(c52653092.xyzop2)
	-- e1:SetOperation(aux.XyzOperation(c52653092.ovfilter,c52653092.xyzop2))
	e1:SetOperation(c52653092.xyzop2)
...
function c52653092.ovfilter2(c)
	return c:IsFaceup() and c:IsSetCard(0x48)
end
function c52653092.spcon(e,c)
	if c==nil then return true end
	if Duel.GetLocationCount(c:GetControler(),LOCATION_MZONE)<=0 then return false end
	local g=Duel.GetMatchingGroup(c52653092.ovfilter2,c:GetControler(),LOCATION_MZONE,0,nil)
	local ct=g:GetClassCount(Card.GetRank)
	return ct>2
end

function c52653092.spcon(e,c)
	return Duel.IsExistingMatchingCard(c52653092.cfilter,tp,LOCATION_HAND,0,1,nil) and 
	Duel.IsExistingMatchingCard(c52653092.ovfilter,tp,LOCATION_MZONE,0,1,nil)
end
function c52653092.xyzop2(e,tp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c52653092.cfilter,tp,LOCATION_HAND,0,1,nil) and 
	Duel.IsExistingMatchingCard(c52653092.ovfilter,tp,LOCATION_MZONE,0,1,nil) end
	Duel.DiscardHand(tp,c52653092.cfilter,1,1,REASON_COST+REASON_DISCARD,nil)
	local c=e:GetHandler()
	local og=Duel.SelectMatchingCard(tp,c52653092.ovfilter,tp,LOCATION_MZONE,0,1,1,nil,tp)
