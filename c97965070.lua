...
function c97965070.initial_effect(c)
	...
	p2:SetProperty(EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_UNCOPYABLE)
	...
	p3:SetProperty(EFFECT_FLAG_DAMAGE_STEP+EFFECT_FLAG_CARD_TARGET+EFFECT_FLAG_DELAY+EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_UNCOPYABLE)
	p3:SetRange(LOCATION_MZONE+LOCATION_EXTRA+LOCATION_GRAVE+LOCATION_REMOVED)
	p3:SetProperty(EFFECT_FLAG_CANNOT_DISABLE)
	p3:SetValue(TYPE_SYNCHRO+TYPE_XYZ+TYPE_FUSION+TYPE_PENDULUM+TYPE_RITUAL+TYPE_MONSTER)
	c:RegisterEffect(p3)
	--To Attack
	...
	p4:SetProperty(EFFECT_FLAG_CANNOT_DISABLE)
	...
	p7:SetProperty(EFFECT_FLAG_SINGLE_RANGE+EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_DELAY+EFFECT_FLAG_UNCOPYABLE)
	p7:SetRange(LOCATION_MZONE)
	p7:SetCode(EFFECT_SELF_DESTROY)
	p7:SetCondition(c97965070.descon)
	c:RegisterEffect(p7)
	--Cannot be in Defense Position
	local p8=Effect.CreateEffect(c)
	p8:SetType(EFFECT_TYPE_SINGLE)
	p8:SetProperty(EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_UNCOPYABLE)
	p8:SetCode(EFFECT_CANNOT_CHANGE_POSITION)
	p8:SetRange(LOCATION_MZONE)
	c:RegisterEffect(p8)
	local p9=Effect.CreateEffect(c)
	p9:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
	p9:SetCode(EVENT_CHAIN_SOLVED)
	p9:SetProperty(EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_UNCOPYABLE)
	p9:SetRange(LOCATION_SZONE)
	p9:SetOperation(c97965070.regop)
	c:RegisterEffect(p9)
	local p10=Effect.CreateEffect(c)
	p10:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
	p10:SetCode(EVENT_ADJUST)
	p10:SetProperty(EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_UNCOPYABLE)
	p10:SetRange(LOCATION_SZONE)
	p10:SetOperation(c97965070.regop)
	c:RegisterEffect(p10)
	local p11=Effect.CreateEffect(c)
	p11:SetType(EFFECT_TYPE_SINGLE)
	p11:SetCode(EFFECT_LEVEL_RANK_S)
	p11:SetProperty(EFFECT_FLAG_DELAY+EFFECT_FLAG_UNCOPYABLE)
	p11:SetRange(LOCATION_MZONE)
	c:RegisterEffect(p11)
	--special summon
...
function c97965070.matfilter1(c)
    return c:IsFaceup() and c:IsType(TYPE_MONSTER) and (c:GetLevel()==4 or (c:GetRank()==4 and c:IsType(TYPE_XYZ)))
end
...
function c97965070.repop(e,tp,eg,ep,ev,re,r,rp)
    ...
    e3:SetProperty(EFFECT_FLAG_SPSUM_PARAM+EFFECT_FLAG_UNCOPYABLE+EFFECT_FLAG_CANNOT_DISABLE)
...
function c97965070.regop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
...
function c97965070.discost(e,tp,eg,ep,ev,re,r,rp,chk)
   ...
   local c=e:GetHandler()
	...
	if c:GetLevel()==1 then c:SetStatus(STATUS_NO_LEVEL,true) end
	if c:GetRank()==1 then c:SetStatus(STATUS_NO_RANK,true) end
	local e1=Effect.CreateEffect(c)
	e1:SetType(EFFECT_TYPE_SINGLE)
...
function c97965070.disop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc then
		...
		if tc:GetBaseAttack()>=0 and tc:IsType(TYPE_MONSTER) and c:GetLevel()>0 and Duel.SelectYesNo(tp,aux.Stringid(97965070,0)) then
...
function c97965070.xyzlv(e,c,rc)
	return c:GetRank()
end