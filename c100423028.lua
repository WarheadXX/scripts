...
function s.spcheck(sg,tp)
    return Duel.IsExistingTarget(aux.TRUE,tp,LOCATION_MZONE,LOCATION_MZONE,sg:GetCount(),sg)
end
...
function s.desop(e,tp,eg,ep,ev,re,r,rp)
    ...
    if tg:GetCount()>0 then
		...
		if g:GetCount()>0 and Duel.IsPlayerCanDraw(1-tp,g:GetCount()) and Duel.SelectYesNo(1-tp,aux.Stringid(id,0)) then
			Duel.BreakEffect()
