...
function s.spcheck(sg,tp,exg,dg)
	...
	return dg:GetCount()-a>=sg:GetFirst():GetLink()
end
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then
		...
		e:SetLabel(0)
		return Duel.CheckReleaseGroup(tp,s.cfilter,1,false,s.spcheck,nil,dg)
	end
	local sg=Duel.SelectReleaseGroup(tp,s.cfilter,1,1,false,s.spcheck,nil,dg)
	...
	Duel.SetOperationInfo(0,CATEGORY_DESTROY,dg,dg:GetCount(),tp,0)
end
