...
function s.scfilter1(c,tp,mc)
   ...
   return c:IsFaceup() and not c:IsType(TYPE_XYZ) and not c:IsType(TYPE_LINK) and not c:IsType(TYPE_TUNER) --and 
   -- Duel.IsExistingMatchingCard(Card.IsSynchroSummonable,tp,LOCATION_EXTRA,0,1,nil,mc,mg)
end
...
function s.sctg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsPlayerCanSpecialSummonCount(tp,2)
		...
		and Duel.IsExistingTarget(s.scfilter1,tp,LOCATION_MZONE,0,1,nil,c) end
...
function s.scop(e,tp,eg,ep,ev,re,r,rp)
	...
	--if Duel.IsExistingTarget(Card.IsSynchroSummonable,tp,LOCATION_EXTRA,0,1,nil,nil,mg) then return end
